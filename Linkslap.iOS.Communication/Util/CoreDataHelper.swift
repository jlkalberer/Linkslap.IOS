...
internal class CoreDataHelper: NSObject{
    ...
    internal func all<TModel>(request : NSFetchRequest) -> [TModel] {
        var result = managedObjectContext!.executeFetchRequest(request, error: nil)
    ...
    internal func get<TModel>(request : NSFetchRequest) -> TModel? {
        var output : [TModel] = all(request)
